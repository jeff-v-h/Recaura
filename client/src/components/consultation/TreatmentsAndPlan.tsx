...
class TreatmentsAndPlan extends React.Component<Props> {
  ...
  saveForm = (values: TreatmentsAndPlans) => {
    this.props.modifyTreatmentsAndPlans(values);
  };
  ...
  render() {
    const { treatments, plans, display } = this.props;

    return (
      <>
        <NavPills value={display} onChange={this.onChangeSection} />
        <TreatmentsAndPlanForm formRef={this.formRef} onSubmit={this.saveForm} />
      </>
    );
  }
}

export default compose<React.ComponentType<ParentProps>>(withRouter, connector)(TreatmentsAndPlan);
